    ...
    public class Word_GetAll : WebApiIntegrationTest
    {
        ...
        public async void GetsWordsForWordList()
        {
            ...
            Transact(session =>
            {
                ...
                session.Store(new Word(generator.Generate(), "w2", "d2", wordList.OwnerId, wordList.Id));
                SystemTime.UtcDateTime = () => new DateTime(2010, 1, 1);
                session.Store(new Word(generator.Generate(), "w1", "d1", wordList.OwnerId, wordList.Id));
            });
